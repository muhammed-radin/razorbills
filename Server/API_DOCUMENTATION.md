...
|-----------|------|-------------|---------|
| `search` | string | Text search across title, description, brand, and category | `?search=wireless` |
| `category` | string | Filter by category (case-insensitive) | `?category=Electronics` |
| `brand` | string | Filter by brand (case-insensitive) | `?brand=SoundMagic` |
...
| `sortBy` | string | Field to sort by (e.g., price, title, createdAt) | `?sortBy=price` |
